# Formal Quote Template
## Dentist Appointment Booking App

Quote No: [QUOTE-2026-001]  
Issue Date: [DD Month YYYY]  
Valid Until: [DD Month YYYY]  
Prepared For: [Client Name]  
Prepared By: [Your Name / Business]  
Currency: ZAR (R)

---

## 1) Scope Summary
Design, build, secure, and deploy a production-ready dental appointment booking web app with user accounts and PostgreSQL-backed appointment storage.

## 2) Cost Breakdown

| Item | Description | Amount (ZAR) |
|---|---|---:|
| Development, testing, and live production | End-to-end implementation and production release | R5,000 |
| **Total Project Fee (Once-Off)** |  | **R5,000** |

## 3) Milestone Payment Schedule

| Milestone | Deliverable | % | Amount (ZAR) |
|---|---|---:|---:|
| Milestone 1 | Kickoff and implementation start | 50% | R2,500 |
| Milestone 2 | Testing, launch, and handover | 50% | R2,500 |
| **Total** |  | **100%** | **R5,000** |

Suggested terms:
- Work starts after Milestone 1 payment clearance.
- Each milestone is billed on acceptance of listed deliverables.

## 4) Monthly Support and Marketing Fees

| Plan | Includes | Monthly Fee (ZAR) |
|---|---|---:|
| Maintenance | Bug fixes, checks, and routine upkeep | R1,000 |
| Email + Social Media Marketing | Campaign support and monthly outreach | R1,000 |
| **Total Monthly Retainer** | Maintenance + marketing support | **R2,000** |

Value-first note:
- Pricing is intentionally set to overdeliver value, build trust, and encourage long-term referrals.

## 5) Project Duration
Estimated total duration: 2 to 4 weeks from kickoff and receipt of required assets/content.

## 6) Assumptions
- Client provides final branding, copy, and legal policy text.
- Third-party services (SMS/email) are billed separately by provider.
- Scope changes beyond this quote are handled via change request.

## 7) Exclusions (Unless Added)
- Native mobile app (iOS/Android).
- Complex admin panel beyond agreed scope.
- Advanced integrations (medical aid APIs, ERP, accounting systems).

## 8) Terms and Conditions
- 50% of remaining unpaid amount due before production release if milestone plan is not used.
- Delays from pending client feedback may shift delivery dates.
- Source code transfer occurs after final payment.
- Hosting and third-party subscriptions remain client-owned where applicable.

---

## Acceptance
Client Name: __________________________  
Signature: _____________________________  
Date: _________________________________  

Service Provider: ______________________  
Signature: _____________________________  
Date: _________________________________
